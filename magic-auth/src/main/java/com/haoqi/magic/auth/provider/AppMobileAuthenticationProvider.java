 ...
 */
@Slf4j
public class AppMobileAuthenticationProvider extends AbstractAuthenticationProvider {
    private final PasswordEncoder passwordEncoder;

    public AppMobileAuthenticationProvider(UserDetailsService userDetailsService, PasswordEncoder passwordEncoder) {
    ...
    protected Authentication buildAuthenticationToken(Authentication authentication, UserDetails userDetail) {
        ...
        AppMobileAuthenticationToken mobileAuthenticationToken = new AppMobileAuthenticationToken(username, password, userDetail.getAuthorities());
    ...
    protected UserDetails loadUser(String username) {
        return ((IUserService) getUserDetailsService()).loadUserNotAdminByUsername(username);
    }

    @Override
    public boolean supports(Class<?> authentication) {
        return AppMobileAuthenticationToken.class.isAssignableFrom(authentication);
    }
}
