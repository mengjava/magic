...
@Slf4j
@Data
public class BuYunMarkingSmsUtil {
    ...
    public BuYunSmsResult buYunSendSms(String mobile, String content, String extno, String product) {
        ...
        try {
            if (!TELPHONEREG.matcher(mobile).matches()){
                return new BuYunSmsResult(BuYunSmsResult.codeEnum.CODE_107.getCode(),BuYunSmsResult.codeEnum.CODE_107.getMsg());
            }
            ...
            params.add(new BasicNameValuePair("pswd", appkey));
            ...
            log.info("[BuYunSmsUtil buYunSendSms return code] headCode : {}", headCode);
            if (headCode == 200) {
                String body1 = EntityUtils.toString(ent, Charset.forName("utf-8"));
                return (new BuYunSmsResult()).parseFromHTTPResponse(body1,true);
            }
        } catch (Exception ex) {
